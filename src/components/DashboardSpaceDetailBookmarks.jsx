...
function DashboardSpaceDetailBookmarks({ bookmarks }) {
  return (
    <div className="p-4 flex flex-col gap-4 bg-white rounded-xl shadow-lg">
      <h3 className="font-medium text-slate-900 tracking-tight text-lg">Documents</h3>
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {bookmarks &&
          bookmarks.map((bookmark, index) => {
            return <DashboardSpaceDetailBookmarkCard document={bookmark.document} />;
          })}
      </div>
    </div>
  );
